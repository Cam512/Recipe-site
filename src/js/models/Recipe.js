...
export default class Recipe {
    ...
    async getRecipe() {
        try {
            const res = await axios(`${secretKey.proxy}https://www.food2fork.com/api/get?key=${secretKey.key}&rId=${this.id}`);
    ...
    ParseIngredients() {
        ...
        const newIngredients = this.ingredients.map(el => {
            ...
            if (unitIndex > -1) {
                // There is a unit
                // EX. 3 1/2 cups. arrCount is [4, 1/2] --> "4 + 1/2"
                ...
                // There is NO unit nad NO number in 1st position
                objIng = {
                    count: 1,
                    unit: '',
                    ingredient
